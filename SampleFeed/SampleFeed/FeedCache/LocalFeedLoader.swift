...
public final class FeedCachePolicy {
    ...
    public func validate(timestamp: Date, against date: Date) -> Bool {
...
extension LocalFeedLoader {
    ...
    public func load(completion: @escaping (LoadResult) -> Void) {
        store.retrieve {[weak self] result in
            ...
            switch result {
            case let .found(cache) where self.cachePolicy.validate(timestamp: cache.timestamp, against: self.currentDate()):
